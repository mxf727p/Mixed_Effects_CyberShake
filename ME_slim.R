...
Dataname <- "CyberShake_SC_regression"     # Options: "NGA_W2", "CyberShake", "NGA_W2_SC", "CyberShake_SC_regression", "CyberShake_SC_full"
lon_e <- -115
lon_w <- -122
lat_s <- 31
lat_n <- 37
...
for (i in 1:nrow(coeff)) {
  ...
  if (plot_residual) {
	...
	if (output_flag) {
      		...
      		write.csv(model_P2P,file=paste(path_outputs,"/Path_term.",Dataname, ".T",
                                      formatC(Period,digits=3,width=6,format="f",flag="0"),".csv", sep=""), row.names=TRUE)
      		write.csv(model_eps3,file=paste(path_outputs,"/Aleatory_residuals.",Dataname, ".T",
                                formatC(Period,digits=3,width=6,format="f",flag="0"),".csv", sep=""), row.names=TRUE)
  	}
    	toc()
  ...
  if (output_flag) {
      lon <- aggregate(Dataset[["lon"]],list(Dataset$eqid),mean)
      colnames(lon) <- c("eqid","lon")
      lat <- aggregate(Dataset[["lat"]],list(Dataset$eqid),mean)
      colnames(lat) <- c("eqid","lat")
      model4_L2L <- cbind(model4_L2L, lon, lat)
      ...
      model4_S2S <- cbind(model4_S2S, st_lon, st_lat)
      ...
      model4_P2P <- cbind(model4_P2P, st_lon, st_lat, lon, lat)
