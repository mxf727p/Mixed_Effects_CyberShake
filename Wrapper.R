...
for (i in 1:nrow(coeff)) {
  ...
  if (L2L_flag) {
    model <- lme(fixed = Resi ~ 1, data = Dataset, method="REML", random = ~ 1 | eqid)
    ...
    if (GM_model == "VIL") {
      model2 <- nlme(obs ~ lnSa2(mag, rrup, vs30, b1, b2, b3, b4, b5, b6),
                     data = Dataset[Dataset$obs!="NaN",],
                     fixed = b1 + b2 + b3 + b4 + b5 + b6 ~ 1,
                     random = b1 ~ 1 | eqid,
                     start=c(b1=-6, b2=3, b3=0.0, b4=-2.6, b5=0.3, b6=-1.29))
      ...
      model2 <- nlme(obs ~ lnSa2(mag, rrup, c1, c2, c4, c6, c7, c10, c11),
                       data = Dataset[Dataset$obs!="NaN",],
                       fixed = c1 + c2 + c4 + c6 + c7 + c10 + c11 ~ 1,
                       random = c1 ~ 1 | eqid,
    ...
    model3 <- lmer(Resi ~ 1 + (1|eqid), data=Dataset, REML=TRUE)
    ...
    if (GM_model == "VIL") {
        ...
        lnSa.deriv = deriv(~ c1 + c2 * mag + (c6 + c7 * mag) * log(rrup + exp(c4)) + c10 * (mag - 6) ^ 2 + c11 * rrup, 
                           namevec=c('c1','c2','c4','c6','c7','c10','c11'), 
                           function.arg = c('mag','rrup','c1','c2','c4','c6','c7','c10','c11'))
        model4 <- nlmer(obs ~ lnSa.deriv(mag,rrup,c1,c2,c4,c6,c7,c10,c11) ~ (c1|eqid) + (c1|stid) + (c1|pathid),
                          data=Dataset[Dataset$obs!="NaN",], start=c(c1=-6, c2=0.9, c4=0.35, c6=-3.0, c7=0.3, c10=-0.25,c11=-0.004))
    }
  ...
  if (P2P_flag) {
    ...
    ymax <- max(hist(model2_L2L$L2L, breaks = seq(from=-15,to=15,by=0.1), plot=FALSE)$counts)
    p2_1 <- ggplot(model2_L2L, aes(x=L2L)) + geom_histogram(color="black", fill="white", binwidth = 0.1) + scale_x_continuous(lim=x_axis_range) +
      stat_function(fun = function(x) dnorm(x, mean = 0, sd = sqrt(var(model2_L2L$L2L))) * nrow(model2_L2L) * 0.1, color = "red", size = 1) + 
      annotate("text", x=-1, y=ymax*0.1, label = paste0("phi[L2L] ==",formatC(sqrt(var(model2_L2L$L2L)),digits=2,format="f",flag="0")), color="red", parse=TRUE) +
      annotate("text", x=0, y=ymax*1, label = "lmer(Residual ~ 1 + 1 | EQID)", color="blue", size=3.5) +
      annotate("text", x=0, y=ymax*0.9, label = "paste(\"lmer(\",delta,W[es],\" ~ 1 + 1 | STID)\")", color="blue", parse=TRUE, size=3.5) +
      annotate("text", x=0, y=ymax*0.8, label = "paste(\"lmer(\",delta,WS[es],\" ~ 1 + 1 | PATHID)\")", color="blue", parse=TRUE, size=3.5)
    
    ymax <- max(hist(model2_S2S$S2S, breaks = seq(from=-15,to=15,by=0.1), plot=FALSE)$counts)
    p2_2 <- ggplot(model2_S2S, aes(x=S2S)) + geom_histogram(color="black", fill="white", binwidth = 0.1) + scale_x_continuous(lim=x_axis_range) +
      stat_function(fun = function(x) dnorm(x, mean = 0, sd = sqrt(var(model2_S2S$S2S))) * nrow(model2_S2S) * 0.1, color = "red", size = 1) + 
      annotate("text", x=-1, y=ymax*0.1, label = paste0("phi[S2S] ==",formatC(sqrt(var(model2_S2S$S2S)),digits=2,format="f",flag="0")), color="red", parse=TRUE)
    
    ymax <- max(hist(model2_P2P$P2P, breaks = seq(from=-15,to=15,by=0.1), plot=FALSE)$counts)
    p2_3 <- ggplot(model2_P2P, aes(x=P2P)) + geom_histogram(color="black", fill="white", binwidth = 0.1) + scale_x_continuous(lim=x_axis_range) +
      stat_function(fun = function(x) dnorm(x, mean = 0, sd = sqrt(var(model2_P2P$P2P))) * nrow(model2_P2P) * 0.1, color = "red", size = 1) + 
      annotate("text", x=-1, y=ymax*0.1, label = paste0("phi[P2P] ==",formatC(sqrt(var(model2_P2P$P2P)),digits=2,format="f",flag="0")), color="red", parse=TRUE)
    ############
    ...
    ymax <- max(hist(model3_L2L$L2L, breaks = seq(from=-15,to=15,by=0.1), plot=FALSE)$counts)
    p3_1 <- ggplot(model3_L2L, aes(x=L2L)) + geom_histogram(color="black", fill="white", binwidth = 0.1) + scale_x_continuous(lim=x_axis_range) +
      stat_function(fun = function(x) dnorm(x, mean = 0, sd = sqrt(var(model3_L2L$L2L))) * nrow(model3_L2L) * 0.1, color = "red", size = 1) + 
      annotate("text", x=-1, y=ymax*0.1, label = paste0("phi[L2L] ==",formatC(sqrt(var(model3_L2L$L2L)),digits=2,format="f",flag="0")), color="red", parse=TRUE) +
      annotate("text", x=0, y=ymax*1, label = "lmer(Residual ~ 1 + 1 | EQID", color="blue", size=3.5) +
      annotate("text", x=0, y=ymax*0.9, label = "+ 1 | STID + 1 | PATHID)", color="blue", size=3.5) 
    
    ymax <- max(hist(model3_S2S$S2S, breaks = seq(from=-15,to=15,by=0.1), plot=FALSE)$counts)
    p3_2 <- ggplot(model3_S2S, aes(x=S2S)) + geom_histogram(color="black", fill="white", binwidth = 0.1) + scale_x_continuous(lim=x_axis_range) +
      stat_function(fun = function(x) dnorm(x, mean = 0, sd = sqrt(var(model3_S2S$S2S))) * nrow(model3_S2S) * 0.1, color = "red", size = 1) + 
      annotate("text", x=-1, y=ymax*0.1, label = paste0("phi[S2S] ==",formatC(sqrt(var(model3_S2S$S2S)),digits=2,format="f",flag="0")), color="red", parse=TRUE)
    
    ymax <- max(hist(model3_P2P$P2P, breaks = seq(from=-15,to=15,by=0.1), plot=FALSE)$counts)
    p3_3 <- ggplot(model3_P2P, aes(x=P2P)) + geom_histogram(color="black", fill="white", binwidth = 0.1) + scale_x_continuous(lim=x_axis_range) +
      stat_function(fun = function(x) dnorm(x, mean = 0, sd = sqrt(var(model3_P2P$P2P))) * nrow(model3_P2P) * 0.1, color = "red", size = 1) + 
      annotate("text", x=-1, y=ymax*0.1, label = paste0("phi[P2P] ==",formatC(sqrt(var(model3_P2P$P2P)),digits=2,format="f",flag="0")), color="red", parse=TRUE)
    ...
    if (GM_model == "VIL") {
        ...
        ggsave(file=paste(path_figures,"/Residual_P2P_",Dataname,".T", formatC(Period,digits=3,width=6,format="f",flag="0"),".png",sep=""), 
               g,height=9, width=12, units='in', dpi=600)
    ############
    ...
    if (GM_model == "VIL") {
        ...
        ggsave(file=paste(path_figures,"/Compare_P2P_",Dataname,".T", formatC(Period,digits=3,width=6,format="f",flag="0"),".png",sep=""), 
               g,height=6, width=6, units='in', dpi=600)
    ...
    yc_list <- seq(from=32.6, to=35, by=0.1)
    if (GM_model == "VIL") {
        ...
        ave_data <- data.frame(matrix(NA, nrow = length(xc_list)*length(yc_list), ncol = 3))
  ...
  if (output_flag) {    
      write.csv(model4_L2L,file=paste(path_outputs,"/Source_term.",Dataname, ".T", 
                                      formatC(Period,digits=3,width=6,format="f",flag="0"),".csv", sep=""), row.names=TRUE)
      write.csv(model4_S2S,file=paste(path_outputs,"/Site_term.",Dataname, ".T", 
                                      formatC(Period,digits=3,width=6,format="f",flag="0"),".csv", sep=""), row.names=TRUE)
      write.csv(model4_P2P,file=paste(path_outputs,"/Path_term.",Dataname, ".T", 
                                      formatC(Period,digits=3,width=6,format="f",flag="0"),".csv", sep=""), row.names=TRUE)
      write.csv(epsR,file=paste(path_outputs,"/Aleatory_residuals.",Dataname, ".T", 
                                formatC(Period,digits=3,width=6,format="f",flag="0"),".csv", sep=""), row.names=TRUE)
  }
  ...
  if (plot_residual) {
    ...
    toc()
    tic("Compare residual")
    Compare_residual_Villani(Dataname, Dataset, model_L2L, model_eps, path_figures, Period)
    toc()
  }
  # End plot figures
