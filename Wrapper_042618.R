...
GM_model <- "KOT"        # Options: "ASK14", "KOT", "VIL"
path_main <- setwd("/Users/xmeng/Documents/MyWork/CyberShake")
regression_flag <- 1
if (regression_flag) {
  regression_model <- 3   # 1: nlme; 2: nls; 3: nlsLM; 4: nlmer
}
residual_flag <- 0
if (residual_flag == 0) {
  read_residual <- 0
...
for (i in 1:nrow(coeff)) {
  ...
  if (P2P_flag) {
    ...
    p1_1 <- ggplot(model_L2L, aes(x=L2L)) + geom_histogram(color="black", fill="white", binwidth = 0.1) + scale_x_continuous(lim=c(-1,1)) +
            ...
            annotate("text", x=0, y=1300, label = "lme(Residual ~ 1 + 1 | EQID)", color="blue", size=2.5) +
            annotate("text", x=0, y=1200, label = "paste(\"lme(\",delta,W[es],\" ~ 1 + 1 | STID)\")", color="blue", parse=TRUE, size=2.5) +
            annotate("text", x=0, y=1100, label = "paste(\"lme(\",delta,WS[es],\" ~ 1 + 1 | PATHID)\")", color="blue", parse=TRUE, size=2.5)
    ...
    p2_1 <- ggplot(model2_L2L, aes(x=L2L)) + geom_histogram(color="black", fill="white", binwidth = 0.1) + scale_x_continuous(lim=c(-1,1)) +
      ...
      annotate("text", x=0, y=1300, label = "lmer(Residual ~ 1 + 1 | EQID)", color="blue", size=2.5) +
      annotate("text", x=0, y=1200, label = "paste(\"lmer(\",delta,W[es],\" ~ 1 + 1 | STID)\")", color="blue", parse=TRUE, size=2.5) +
      annotate("text", x=0, y=1100, label = "paste(\"lmer(\",delta,WS[es],\" ~ 1 + 1 | PATHID)\")", color="blue", parse=TRUE, size=2.5)
    ...
    p3_1 <- ggplot(model3_L2L, aes(x=L2L)) + geom_histogram(color="black", fill="white", binwidth = 0.1) + scale_x_continuous(lim=c(-1,1)) +
      ...
      annotate("text", x=0, y=1300, label = "lmer(Residual ~ 1 + 1 | EQID", color="blue", size=2.5) +
      annotate("text", x=0, y=1200, label = "+ 1 | STID + 1 | PATHID)", color="blue", size=2.5) 
    ...
    if (GM_model == "VIL") {
        ...
        p4_1 <- ggplot(model4_L2L, aes(x=L2L)) + geom_histogram(color="black", fill="white", binwidth = 0.1) + scale_x_continuous(lim=c(-1,1)) +
          ...
          annotate("text", x=0, y=1300, label = "nlmer(obs ~ deriv(lnSa) ~ 1 | EQID", color="blue", size=2.5) +
          annotate("text", x=0, y=1200, label = "+ 1 | STID + 1 | PATHID)", color="blue", size=2.5) 
